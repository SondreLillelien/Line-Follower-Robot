...
def get_hough_lines(img):
    ...
    hough_lines = cv.HoughLinesP(roi, rho=RHO_RESOLUTION, theta=THETA_RESOLUTION, threshold=VOTE_THRESHOLD, maxLineGap=MAX_LINE_GAP, minLineLength=MIN_LINE_LENGTH)
    return hough_lines
...
def get_control_line(lines):
    ...
    return [int(heading_x1), int(heading_y1), int(heading_x2), int(heading_y2), lines[0][4], int(heading_y1-lines[0][4]*heading_x1)]
...
def get_filtered_lines(lines):
    ...
    for line in lines:
        if line[6] == "right": right_edge_lines.append(line)
        elif line[6] == "left": left_edge_lines.append(line)
